...
import { FiArrowLeft, FiHeart, FiStar, FiMapPin, FiUsers, FiHome, FiCheck, FiChevronLeft, FiChevronRight, FiShare2, FiSun, FiWind, FiMessageSquare } from 'react-icons/fi'
...
export default function PropertyDetail() {
  ...
  const { allProperties, isWishlisted, toggleWishlist, trackView, createBooking, getPropertyReviews, getPropertyAverageRating } = useAppData()
  const { user } = useAuth()
  const { showToast } = useToast()

  const property = allProperties.find(p => String(p.id) === String(id))
  const propIdKey = !isNaN(id) ? Number(id) : id
  const liveReviews = getPropertyReviews(propIdKey)
  const avgRating = getPropertyAverageRating(propIdKey)
