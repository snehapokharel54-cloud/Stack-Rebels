...
export const createBooking = async (req, res) => {
  try {
    ...
    if (booking_type === "instant" && !listing.instant_book_enabled)
      return res
        .status(400)
        .json({ success: false, message: "Instant booking not allowed" });
    if (listing.host_id === guestId)
      return res
        .status(400)
        .json({ success: false, message: "Cannot book your own listing!" });
    ...
    const tax = Math.round(
      (base_price + cleaning_fee + platform_service_fee) * 0.13,
    ); // 13% VAT
    const total = Math.round(
      base_price + cleaning_fee + platform_service_fee + tax,
    );
...
export const cancelGuestBooking = async (req, res) => {
  try {
    ...
    const listingRes = await query("SELECT title FROM listings WHERE id = $1", [
      booking.listing_id,
    ]);
    ...
    await query(
      ...
      [
        booking.host_id,
        "Booking Cancelled ❌",
        `The booking for ${listingTitle} has been cancelled by the guest.${reason ? ` Reason: ${reason}` : ""}`,
        "booking_cancelled",
      ],
    );
    ...
    await query(
      ...
      [
        guestId,
        "Trip Cancelled ❌",
        `You have cancelled your trip to ${listingTitle}.${reason ? ` Reason: ${reason}` : ""}`,
        "booking_cancelled",
      ],
    );
    ...
    try {
      ...
      console.error("Guest cancel email failed:", err.message);
    }
...
export const getBookingPriceBreakdown = async (req, res) => {
  try {
    /* Simple stub since it's injected tightly into createBooking as well */
    res.json({
      success: true,
      data: { base: 100, fees: 20, taxes: 10, total: 130 },
    });
  } catch (err) {}
};
...
export const acceptBooking = async (req, res) => {
  try {
    ...
    if (result.rows.length === 0)
      return res
        .status(400)
        .json({ success: false, message: "Unable to confirm" });
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        `Unfortunately, the host had to cancel your booking for ${listingTitle}.${reason ? ` Reason: ${reason}` : ""}`,
        "booking_cancelled",
      ],
    );
    ...
    await query(
      ...
      [
        hostId,
        "Booking Cancelled ❌",
        `You cancelled the booking for ${listingTitle}.${reason ? ` Reason: ${reason}` : ""}`,
        "booking_cancelled",
      ],
    );
