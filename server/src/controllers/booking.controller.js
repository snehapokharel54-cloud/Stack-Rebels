...
export const createBooking = async (req, res) => { 
  try {
    ...
    const listingQuery = await query(
      "SELECT host_id, price_per_night, cleaning_fee, status, instant_book FROM listings WHERE id = $1",
      [listing_id]
    );
    ...
    if (booking_type === 'instant' && !listing.instant_book) return res.status(400).json({ success: false, message: "Instant booking not allowed" });
    ...
    const base_price = nights * listing.price_per_night;
    const cleaning_fee = listing.cleaning_fee || 0;
    ...
    const total = base_price + cleaning_fee + platform_service_fee + tax;
    ...
    const result = await query(
      `INSERT INTO bookings (listing_id, host_id, guest_id, check_in, check_out, nights, status, booking_type, price_breakdown, payment_status, special_requests)
      ...
      [listing_id, listing.host_id, guestId, check_in, check_out, nights, initStatus, booking_type, price_breakdown, special_requests]
    );
...
export const getGuestBookings = async (req, res) => { 
  try {
    ...
    let sql = `SELECT b.id as booking_id, l.title as listing_title, l.photos[1] as listing_photo,
  ...
  try {
    ...
    const result = await query(
      `SELECT b.id as booking_id, 
              json_build_object('id', l.id, 'title', l.title, 'photos', l.photos) as listing,
...
export const cancelGuestBooking = async (req, res) => { 
  try {
    ...
    res.json({
      ...
      refund: { eligible: true, amount: 0, policy_applied: "moderate", refund_eta_days: 5 }
    });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
...
export const cancelHostBooking = async (req, res) => { res.json({ success: true, message: 'Booking cancelled by host' }); };