...
export const updatePricing = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const { price_per_night, cleaning_fee, weekly_discount, monthly_discount } =
      req.body;
...
const stub = (name) => async (_req, res) => {
  res
    .status(501)
    .json({ success: false, message: `${name} is not yet implemented.` });
};
...
export const getCalendar = async (_req, res) =>
  res.json({ success: true, data: { blocked_dates: [] } });
...
export const getCohosts = async (_req, res) =>
  res.json({ success: true, data: [] });
